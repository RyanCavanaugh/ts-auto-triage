...
Publish newspaper reports to GitHub gists. Takes the reports generated by `make-news` and uploads them as gists with:
- Gist filename format: `owner.repo.YYYY-MM-DD.md` (e.g., `microsoft.TypeScript.2025-01-15.md`)
- Automatic creation or update of existing gists
- A news index gist (`owner.repo.news-index.md`) that links to all published reports
...
**Output:**
- Creates or updates gists for each daily report
- Creates or updates a news index gist with links to all reports
- News index is sorted by date (newest first)
- Preserves links to prior reports even if they're not on disk

**Prerequisites:**
- Reports must be generated first using `hereby make-news`
