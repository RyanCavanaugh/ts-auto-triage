...
import { CacheEntrySchema } from './schemas.js';
/**
 * Creates a hash-based key for caching
 */
export function createCacheKey(input, endpoint) {
 ...
 */
export function splitCacheKey(key) {
    if (key.length < 4) {
 ...
 */
export function createKVCache(basePath = '.kvcache') {
    /**
     * Memoized function that caches results of expensive operations
     */
    async function memoize(key, compute, options = {}) {
     ...
     */
    async function clearExpired(maxAgeHours = 24) {
