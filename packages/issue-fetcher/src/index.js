...
import { IssueSchema, CommentSchema, EventSchema } from './schemas.js';
/**
 ...
 */
export function createIssueFetcher(options) {
    const { token, dataPath = '.data', logger = console, rateLimit = { maxRetries: 3, backoffMs: 5000 } } = options;
    const octokit = new Octokit({ auth: token });
    /**
     * Ensures directory exists for storing issue data
     */
    async function ensureDataDir(owner, repo) {
     ...
     */
    function getIssueFilePath(owner, repo, number) {
     ...
     */
    async function fetchComments(owner, repo, number) {
        const comments = [];
        let page = 1;
        const perPage = 100;
        while (true) {
            const response = await withRetry(() => octokit.rest.issues.listComments({
                owner,
                repo,
                issue_number: number,
                page,
     ...
     */
    async function fetchEvents(owner, repo, number) {
        const events = [];
        let page = 1;
        const perPage = 100;
        while (true) {
            const response = await withRetry(() => octokit.rest.issues.listEvents({
                owner,
                repo,
                issue_number: number,
                page,
     ...
     */
    async function fetchIssue(issueRef, force = false) {
        ...
        // Fetch issue data
        const issueResponse = await withRetry(() => octokit.rest.issues.get({
            owner,
            repo,
            issue_number: number
        }));
        ...
        const completeIssue = IssueSchema.parse({
            id: issue.id,
            number: issue.number,
            title: issue.title,
            body: issue.body,
    ...
    async function fetchAllIssues(owner, repo) {
        ...
        while (true) {
            ...
            const response = await withRetry(() => octokit.rest.issues.listForRepo({
                owner,
                repo,
                state: 'all',
                page,
     ...
     */
    async function listCachedIssues(owner, repo) {
        const dir = join(dataPath, owner.toLowerCase(), repo.toLowerCase());
        try {
