...
export function createPRFetcher(
  ...
  return {
    async fetchPR(ref: IssueRef): Promise<GitHubIssue> {
      ...
      try {
        ...
        if (error instanceof Error && 'status' in error && error.status === 403) {
          ...
          return await this.fetchPR(ref);
        }
        throw new Error(`Failed to fetch PR ${ref.owner}/${ref.repo}#${ref.number}: ${error}`);
      }
    },

    async fetchAllPRs(owner: string, repo: string, force: boolean = false): Promise<void> {
      ...
      while (hasNextPage) {
        try {
          ...
          for (const pr of prs.nodes) {
            ...
            if (!force) {
              const localPR = await this.getLocalPR(ref);
    ...
    async getLocalPR(ref: IssueRef): Promise<GitHubIssue | null> {
      const dataPath = createIssueDataPath(ref);
    ...
    async hasLocalPR(ref: IssueRef): Promise<boolean> {
      const pr = await this.getLocalPR(ref);
      return pr !== null;
    },
  };
}
