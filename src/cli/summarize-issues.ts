  ...
  try {
    ...
    for (const { owner, repo } of repoList) {
      ...
      try {
        ...
        logger.warn(`No issue data found in ${dataDir}. Run fetch-issues first. Skipping...`);
        continue;
      }

      logger.info(`Found ${issueFiles.length} issue files to process for ${owner}/${repo}`);
      ...
      logger.info(`Repository ${owner}/${repo} complete! Processed: ${processedCount}, Skipped: ${skippedCount}, Total: ${issueFiles.length}`);
    }
