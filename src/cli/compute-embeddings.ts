  ...
  try {
    ...
    for (const { owner, repo } of repoList) {
      ...
      if (issueKeys.length === 0) {
        logger.warn(`No summaries found for repository ${owner}/${repo}. Run summarize-issues for this repository first. Skipping...`);
        continue;
      }

      logger.info(`Found ${issueKeys.length} issues with summaries to process for ${owner}/${repo}`);
      ...
      logger.info(`Repository ${owner}/${repo} complete! Processed: ${processedCount}, Skipped: ${skippedCount}, Total: ${issueKeys.length}`);
    }
