  ...
  try {
    ...
    if (args.length === 0) {
      ...
      if (!config.github.repos || config.github.repos.length === 0) {
        ...
        console.error('Example: fetch-issues --force Microsoft/TypeScript');
        console.error('Example: fetch-issues Microsoft/TypeScript facebook/react');
        console.error('');
        console.error('Options:');
        console.error('  --force    Force re-fetch of all issues, even if they appear up-to-date');
        console.error('');
    ...
    if (force) {
      logger.info('Force mode enabled - will re-fetch all issues');
    }
