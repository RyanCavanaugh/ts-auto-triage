...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (force) {
      logger.info('Force mode enabled - will re-fetch all recent issues and PRs regardless of cache status');
    }
    ...
    for (const [owner, repo] of repos) {
      ...
      try {
        // Fetch recent issues (last 2 weeks)
        await issueFetcher.fetchRecentIssues(owner, repo, force);
        logger.info(`Completed fetching recent issues for ${owner}/${repo}`);
        
        // Fetch recent PRs (last 2 weeks)
        await prFetcher.fetchRecentPRs(owner, repo, force);
        logger.info(`Completed fetching recent PRs for ${owner}/${repo}`);
      } catch (error) {
